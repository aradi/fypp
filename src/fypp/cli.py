...
def get_option_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument('-D', '--define', action='append', dest='defines',
                        ...
                        help="""
                        define variable, equivalent to the -D pre-processor flags. Depending on
                        the value of -d/--define-value-type, the value is either a string
                        or a Python expression
                        """)
    parser.add_argument('-P', '--python-define', action='append', dest='python_defines',
                        metavar='VAR[=VALUE]', default=defs.defines,
                        help="""
                        equivalent to -D/--define, but interprets the string value as
                        a Python expression
                        """)
    parser.add_argument('-d', '--define-value-type', dest='define_type',
                        choices=['str', 'python'], default=defs.define_type,
                        help="""
                        whether to parse -D/--define values as string or Python expressions.
                        Default is 'python' to preserve backwards compatibility, but **NOTE**,
                        this will be reversed in 4.0 and later.
                        """)
